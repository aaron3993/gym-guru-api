...
export async function handler(event: any) {
    ...
    for (const record of event.Records) {
        ...
        try {
            const workoutPlan = mockWorkoutPlan
            // const workoutPlan = await fetchWorkoutPlanFromOpenAI(messages, openAIAPIKey);
            const routineId = await saveCompleteWorkoutInfo(db, userId, workoutPlan, criteria);
